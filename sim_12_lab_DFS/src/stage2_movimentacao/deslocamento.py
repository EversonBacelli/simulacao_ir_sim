...
def motionRobot(objetivo, list, inicio, m, env):
    obj = objetivo
    # Definir Objetivo no IR-SIM
    env = definirObjetivo(obj, m, env)
    
    listaDeNos = remontarLista(inicio, list)
    eq = m[inicio[0]][inicio[1]]
    env = movimentacaoIR_SIM(eq, env)
    ...
    while True:
        ...
        if atual.posicao == obj:
            env = movimentacaoIR_SIM(atual, env)
...
def definirObjetivo(objetivo, m, env):
